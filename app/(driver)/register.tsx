...
const DriverRegister = () => {
  ...
  const promptDocumentSource = (type: DocumentType) => {
    Alert.alert(`Upload ${DOCUMENT_LABELS[type]}`, "Choose an option", [
  ...
  const validateStep = (): boolean => {
    ...
    if (step === 2) {
      ...
      if (missing.length > 0) {
        Alert.alert(
          "Missing Documents",
          `Please upload: ${missing.map((d) => DOCUMENT_LABELS[d]).join(", ")}`,
        );
        return false;
      }
      return true;
      ...
      >
        <ScrollView contentContainerClassName="pb-12" keyboardShouldPersistTaps="handled">
          ...
          <View className="flex-row items-center px-5 pb-2 pt-4">
            ...
            <Text
              className="ml-3 font-JakartaBold text-lg text-slate-900 dark:text-dark-text"
              {...a11yHeader("Become a Driver")}
            >
              Become a Driver
            </Text>
          </View>
            ...
            <View className="flex-row gap-2">
              {STEPS.map((label, i) => (
                <View key={label} className="flex-1">
                  <View
                    className={`h-1.5 rounded-full ${i <= step ? "bg-primary-500" : "bg-slate-200 dark:bg-dark-border"}`}
                  />
                  <Text
                    className={`mt-1 font-JakartaMedium text-[10px] ${i === step ? "text-primary-500" : "text-slate-400 dark:text-dark-text-secondary"}`}
                  >
                    {label}
                  </Text>
                </View>
          ...
          <View className="mt-6 px-5">
            {step === 0 && (
              <View>
                <Text className="mb-1 font-JakartaBold text-xl text-slate-900 dark:text-dark-text">
                  Personal Information
                </Text>
                <Text className="mb-6 font-JakartaMedium text-sm text-slate-500 dark:text-dark-text-secondary">
                  Tell us about yourself
                </Text>

                <Text className="mb-2 font-JakartaBold text-xs uppercase tracking-wide text-slate-400 dark:text-dark-text-secondary">
                  First Name
                </Text>
                <TextInput
                  ...
                  className="mb-4 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3.5 font-JakartaMedium text-base text-slate-900 dark:border-dark-border dark:bg-dark-card dark:text-dark-text"
                  {...a11y("First name")}
                />
                ...
                <TextInput
                  ...
                  autoCapitalize="none"
                  placeholderTextColor={isDark ? "#4a4a4a" : "#94a3b8"}
                  className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3.5 font-JakartaMedium text-base text-slate-900 dark:border-dark-border dark:bg-dark-card dark:text-dark-text"
                  {...a11y("Email address")}
                />
              </View>
            )}
            ...
            {step === 1 && (
              <View>
                ...
                <Text className="mb-3 font-JakartaBold text-xs uppercase tracking-wide text-slate-400 dark:text-dark-text-secondary">
                  Vehicle Type
                </Text>
                  ...
                  {VEHICLE_TYPES.map((vt) => (
                    <TouchableOpacity
                      ...
                      className={`flex-1 items-center rounded-xl border py-4 ${
                        form.vehicleType === vt.value
                          ...
                          : "border-slate-200 bg-slate-50 dark:border-dark-border dark:bg-dark-card"
                      }`}
                    ...
                    >
                      <Ionicons
                        ...
                        size={24}
                        color={
                          form.vehicleType === vt.value ? "#0286FF" : isDark ? "#F5F5F7" : "#0F172A"
                        }
                      />
                      <Text
                        className={`mt-2 font-JakartaBold text-sm ${
                          form.vehicleType === vt.value
                            ? "text-primary-500"
              ...
              <View>
                ...
                {REQUIRED_DOCUMENTS.map((docType) => (
                  <TouchableOpacity
                    ...
                    className="mb-4 flex-row items-center rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-dark-border dark:bg-dark-card"
                    {...a11yButton(
                  ...
                  >
                    {form.documents[docType] ? (
                      ...
                      <View className="h-16 w-16 items-center justify-center rounded-lg bg-slate-200 dark:bg-dark-border">
                    ...
                    <View className="ml-4 flex-1">
                      ...
                      <Text className="mt-0.5 font-JakartaMedium text-xs text-slate-500 dark:text-dark-text-secondary">
                        {form.documents[docType] ? "Uploaded — tap to replace" : "Tap to upload"}
                      </Text>
                    </View>
                    <Ionicons
            ...
            {step === 3 && (
              <View>
                ...
                <Text className="mt-4 text-center font-JakartaMedium text-xs text-slate-400 dark:text-dark-text-secondary">
        ...
        {/* Bottom action */}
        <View className="border-t border-slate-100 bg-white px-5 pb-6 pt-3 dark:border-dark-border dark:bg-dark-bg">
          {step < STEPS.length - 1 ? (
            <CustomButton title="Continue" onPress={handleNext} className="bg-primary-500" />
          ) : (
            <CustomButton
...
const ReviewSection = ({
  ...
  <View className="mb-5 rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-dark-border dark:bg-dark-card">
    <View className="mb-3 flex-row items-center justify-between">
      <Text className="font-JakartaBold text-sm text-slate-900 dark:text-dark-text">{title}</Text>
      <TouchableOpacity onPress={onEdit} {...a11yButton("Edit", `Edit ${title}`)}>
        <Text className="font-JakartaBold text-xs text-primary-500">Edit</Text>
      </TouchableOpacity>
    </View>
    {children}
  </View>
...
const ReviewRow = ({ label, value }: { label: string; value: string }) => (
  <View className="flex-row justify-between py-1.5">
    ...
    <Text className="font-JakartaMedium text-sm text-slate-900 dark:text-dark-text">{value}</Text>
  </View>
);
