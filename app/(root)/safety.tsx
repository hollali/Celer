  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50 dark:bg-dark-bg">
      ...
      <ScrollView className="px-5" showsVerticalScrollIndicator={false}>
        <View className="mt-5 rounded-2xl bg-red-600 p-4 dark:bg-red-700">
          <Text className="font-JakartaBold text-white">Safety Toolkit Ready</Text>
          ...
          >
            <Text className="font-JakartaBold text-red-600">Emergency SOS</Text>
          </TouchableOpacity>
        </View>
        ...
        <View className="mt-4 overflow-hidden rounded-2xl border border-slate-100 bg-white dark:border-dark-border dark:bg-dark-card">
          <View className="flex-row items-center justify-between px-4 py-4">
            <View className="flex-1 pr-3">
              ...
              <Text className="text-xs text-slate-500 dark:text-dark-text-secondary">
                Let trusted contacts track your ride live.
              </Text>
            </View>
            <Switch
              value={shareTrip}
              onValueChange={setShareTrip}
              {...a11ySwitch("Share trip status", shareTrip)}
            />
          </View>
          ...
          <View className="flex-row items-center justify-between px-4 py-4">
            <View className="flex-1 pr-3">
              ...
              <Text className="text-xs text-slate-500 dark:text-dark-text-secondary">
                Notify contacts automatically if SOS is pressed.
              </Text>
            </View>
            <Switch
              value={emergencyAlerts}
              onValueChange={setEmergencyAlerts}
              {...a11ySwitch("Emergency alerts", emergencyAlerts)}
            />
          </View>
          <View className="h-px bg-slate-100 dark:bg-dark-border" />
          <View className="flex-row items-center justify-between px-4 py-4">
            <View className="flex-1 pr-3">
              ...
              <Text className="text-xs text-slate-500 dark:text-dark-text-secondary">
                Store encrypted clips for safety reports.
              </Text>
            </View>
            <Switch
              value={audioRecording}
              onValueChange={setAudioRecording}
              {...a11ySwitch("Record ride audio", audioRecording)}
            />
          </View>
        </View>
        ...
        <View className="mt-4 rounded-2xl border border-slate-100 bg-white p-4 dark:border-dark-border dark:bg-dark-card">
          <View className="flex-row items-center justify-between">
            ...
            <TouchableOpacity onPress={addContact} {...a11yButton("Add trusted contact")}>
              <Text className="font-JakartaMedium text-blue-600 dark:text-blue-400">+ Add</Text>
            </TouchableOpacity>
          </View>
          <View className="mt-3 gap-2">
            {contacts.map((contact) => (
              <View
                key={contact}
                className="flex-row items-center justify-between rounded-xl border border-slate-200 px-3 py-3 dark:border-dark-border"
              >
                <View className="flex-row items-center">
                  <View className="h-8 w-8 items-center justify-center rounded-full bg-slate-100 dark:bg-dark-bg">
                    <Ionicons
                      name="person-outline"
                      size={16}
                      color={isDark ? "#F5F5F7" : "#334155"}
                    />
                  </View>
                  <Text className="ml-2 font-JakartaMedium text-slate-800 dark:text-dark-text">
                    {contact}
                  </Text>
                </View>
                <TouchableOpacity
                  onPress={() => setContacts((prev) => prev.filter((item) => item !== contact))}
                  {...a11yButton(`Remove ${contact}`)}
                >
        ...
        <View className="mb-8 mt-4 rounded-2xl border border-slate-100 bg-white p-4 dark:border-dark-border dark:bg-dark-card">
          <Text className="font-JakartaMedium text-slate-900 dark:text-dark-text">
            Ride check-in reminder
          </Text>
          <Text className="mt-1 text-xs text-slate-500 dark:text-dark-text-secondary">
            Send a prompt while on trip to confirm you're okay.
          </Text>
          <View
            className="mt-3 flex-row gap-2"
            accessibilityLabel="Check-in reminder interval"
          ...
          <TouchableOpacity
            className="mt-4 flex-row items-center justify-center rounded-xl bg-slate-100 py-3 dark:bg-dark-card"
            onPress={() => {
              Alert.alert(
                "Safety Check",
          ...
          >
            <Ionicons
              ...
              color={isDark ? "#F5F5F7" : "#334155"}
            />
            <Text className="ml-2 font-JakartaMedium text-slate-700 dark:text-dark-text">
